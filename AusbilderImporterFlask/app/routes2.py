...
def index():
    ...
    csv_path = get_latest_csv(os.getcwd())  # Suche nach CSV-Dateien im aktuellen Verzeichnis
    students = []
    if csv_path:
        encoding = detect_encoding(csv_path)
        df = pd.read_csv(csv_path, delimiter=';', encoding=encoding)
        ...
        flash("No CSV files found. Please upload a CSV file.")
...
def filter_csv():
    ...
    base_dir = os.getcwd()  # Suche nach CSV-Dateien im aktuellen Verzeichnis
...
def upload_file():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            classes, blacklist = read_ini_file(ini_path)
            output_path = os.path.join(os.getcwd(), 'AusbilderImportDatei', 'filtered_output.csv')
            filter_csv_by_classes_and_blacklist(file_path, classes, blacklist, output_path)
            return redirect(url_for('index'))
